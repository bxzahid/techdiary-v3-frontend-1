<template>
  <div v-bind="$attrs">
    ...
    >
      ...
      <div>
        <p>{{ $bn($store.state.onlineusers.users.length) }}</p>
      </div>
    </div>
    ...
    <div class="flex flex-col mt-2">
      <nuxt-link
        ...
        class="flex items-center text-dark-secondary"
        v-for="user in $store.state.onlineusers.users"
        :key="user.username"
      >
        <span
          class="inline-block w-2 h-2 mr-2 bg-green-500 rounded-full"
        ></span>
        <span class="inline-block dark:text-gray-300">{{ user.name }}</span>
      </nuxt-link>
    </div>
  </div>
</template>
