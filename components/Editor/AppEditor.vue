<template>
  <div>
    ...
    <div class='max-w-4xl px-3 mx-auto md:px-0 mt-7'>
      <input type='file' class='hidden' ref='thumbnailFileInput'
             accept='.jpeg,.jpg,.png,image/jpeg,image/png' @change='handleUploadImage'>
      <button
        @click='openUploadWindow'
        v-if='!this.article.thumbnail'
        class='flex text-dark px-4 py-1 space-x-1 transition duration-150 text-dark opacity-90 transform focus:outline-none hover:bg-gray-200 dark:hover:bg-gray-500 rounded-md my-4'>
      ...
      <div class='overflow-hidden rounded-md relative' :style='{backgroundImage: `url(${this.article.thumbnail})`}'>
        <img class='object-fit' :src='this.article.thumbnail' v-if='this.article.thumbnail'
             alt='image cover'>
        <button @click='removeThumbnail'
                class='p-2 backdrop-blur backdrop-filter absolute right-4 top-2 rounded-md text-dark bg-red-500 bg-opacity-50 hover:bg-opacity-60'>
      ...
      <input
        type='text'
        class='w-full text-3xl font-bold h-full bg-transparent text-dark focus:outline-none my-3'
