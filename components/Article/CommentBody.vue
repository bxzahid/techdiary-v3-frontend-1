  ...
  <div class='my-7'>
    <div class='flex items-center justify-between'>
      <div class='flex items-center'>
        <img :src='comment.user.profilePhoto' class='bg-gray-300 rounded-full h-10 w-10'>
        <div>
          <p class='font-techdiary ml-3 text-dark'> {{ comment.user.name }}</p>
          <p class='pl-3 text-sm text-dark'> {{ $moment(comment.created_at).fromNow() }} </p>
        </div>
      </div>
      <div class='relative' v-click-away='closeDropdown' v-if='$auth.loggedIn'>
        <svg class='h-10 w-10 text-dark cursor-pointer' viewBox='0 0 25 24' fill='none'
             xmlns='http://www.w3.org/2000/svg' v-if='!editComment'
        ...
        <svg class='h-5 w-5 text-red-500 cursor-pointer' viewBox='0 0 12 11' fill='none'
             xmlns='http://www.w3.org/2000/svg' @click='editComment = false'
             v-if='editComment'>
          <path d='M10.5942 0.5L0.594177 10.5' stroke='#EF4444' stroke-linecap='round' stroke-linejoin='round' />
          <path d='M0.594177 0.5L10.5942 10.5' stroke='#EF4444' stroke-linecap='round' stroke-linejoin='round' />
        </svg>
        <!--Dropdown-->
        <transition name='fade'>
          <div v-if='dropDownOpen' class='comment-body-dropdown' @click.capture='closeDropdown'>
            <button v-if='canBeModified' @click='editComment = true'
                    class='flex items-center text-dark rounded transition duration-200 hover:bg-primary py-1 px-2 hover:text-white w-full'>
              Edit
            </button>
            <button v-if='canBeModified' @click='deleteComment'
                    class='flex items-center text-dark rounded transition duration-200 hover:bg-red-600 py-1 px-2 hover:text-white w-full'>
              Delete
            </button>
          </div>
        </transition>
    ...
    <p class=' my-3 pl-3 text-dark' v-if='!editComment'>{{ comment.body }}</p>
    <div v-if='editComment'>
    <textarea name='editComment'
              class='w-full shadow-md rounded-tl rounded-tr p-2 mb-0 text-dark outline-none dark:bg-gray-700 bg-gray-50'
              rows='5'
              v-model='commentBody'></textarea>
      <button class='bg-primary rounded px-3 py-2' @click='updateComment'>Submit</button>
    </div>
    ...
    <div class='flex items-center justify-between px-3'>
      ...
      <svg class='h-5 w-5 text-dark' viewBox='0 0 16 14' fill='none' xmlns='http://www.w3.org/2000/svg'
        ...
        <svg class='h-5 w-5 text-red-500' viewBox='0 0 12 11' fill='none' xmlns='http://www.w3.org/2000/svg'
             v-if='replyTextarea && showReplyButton'>
<path d='M10.5942 0.5L0.594177 10.5' stroke='#EF4444' stroke-linecap='round' stroke-linejoin='round' />
<path d='M0.594177 0.5L10.5942 10.5' stroke='#EF4444' stroke-linecap='round' stroke-linejoin='round' />
</svg>
    ...
    <div v-if='showReplyButton && replyTextarea'>
      ...
      <textarea name='reply' rows='5' v-model='replyText'
                class='w-full shadow-md rounded-tl rounded-tr p-2 mb-0 outline-none dark:bg-gray-700 bg-gray-50' />
        <div class='bg-gray-300 w-full absolute bottom-0 rounded-bl rounded-br p-1'>
          <svg width='16' height='16' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'>
            ...
            <path d='M12.1023 4.88889L8.21341 1L4.32452 4.88889' stroke='#71717A' stroke-linecap='round'
                  stroke-linejoin='round' />
            <path d='M8.2135 1V10.3333' stroke='#71717A' stroke-linecap='round' stroke-linejoin='round' />
          </svg>
        </div>
      </div>
      <button class='bg-primary rounded px-3 py-2' @click='replyComment'>Submit</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async replyComment() {
      const comment = await this.$axios.$post(`/api/articles/${this.$route.params.articleSlug}/comments`, {
        body: this.replyText,
      ...
      this.$store.commit('comment/CREATE_COMMENT', { comment: newComment, level: this.level + 1 })
      this.replyText = ''
    ...
    async deleteComment() {
      try {
        await this.$axios.$delete(`/api/articles/${this.$route.params.articleSlug}/comments/${this.comment.id}`)
        this.$store.commit('comment/DELETE_COMMENT', { comment: this.comment, level: this.level })
    ...
    async updateComment() {
      await this.$axios.$patch(`/api/articles/${this.$route.params.articleSlug}/comments/${this.comment.id}`, { body: this.commentBody })
...
.comment-body-dropdown {
  @apply absolute right-3 w-20 bg-gray-100 rounded shadow-lg dark:bg-gray-700 top-full;
}
</style>
